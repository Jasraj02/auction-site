      ...
      <div class="form-group">
        ...
        <select class="form-control" id="cat" name="cat">
          ...
          //create dropdown menu for categories
            $searchQuery = "SELECT categoryID, categoryType FROM category ORDER BY categoryType";
...
$searchQuery = "SELECT auction.*,  MAX(bid.bidPrice) as currentPrice, COUNT(bid.bidID) as numberBids
                FROM auction
                LEFT JOIN bid ON auction.auctionID = bid.auctionID
                WHERE auction.endTime>CURRENT_TIMESTAMP()";

if (!empty($keyword)) {
  $searchQuery .= " AND (auction.description LIKE '%$keyword%')";
}
if ($category != 'all') {
  $searchQuery .= " AND auction.categoryID = '$category'";
}
$searchQuery .= " GROUP BY auction.auctionID";

//order results to match sorting option, if multiple items have same value order alphabetically
switch ($ordering) {
  case  'priceLowtoHigh':
  case 'priceLowToHigh':
    $searchQuery .= " ORDER BY COALESCE(MAX(bid.bidPrice), auction.startingPrice) ASC, auction.description ASC";
    break;
  case 'priceHighToLow':
    $searchQuery .= " ORDER BY COALESCE(MAX(bid.bidPrice), auction.startingPrice) DESC, auction.description ASC";
    break;
  case 'date':
    $searchQuery .= " ORDER BY auction.endTime ASC, auction.description ASC";
    break;
  case 'popularityByBids':
    $searchQuery .= " ORDER BY numberBids DESC, auction.description ASC";
    break;
}
...
$paginationQuery = "SELECT COUNT(DISTINCT auction.auctionID) AS numberRows
                    FROM auction
                    LEFT JOIN bid ON auction.auctionID = bid.auctionID
                    WHERE auction.endTime>CURRENT_TIMESTAMP()";

if (!empty($keyword)) {
  $paginationQuery .= " AND (auction.description LIKE '%$keyword%')";
}
if ($category != 'all') {
  $paginationQuery .= " AND auction.categoryID = '$category'";
}
...
//while loop to print a list item for each auction listing retrieved from the query
  ...
  while ($row = mysqli_fetch_assoc($searchResult)) {
    ...
    print_listing_li($row['auctionID'], $row['description'], substr($row['description'], 0, 200) . '...',
